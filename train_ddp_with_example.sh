...
pkill -9 -f biencoder_model_with_example.py || true
sleep 2
...
RUN_NAME="stage1-biencoder-with_example_${CURRENT_TIME}"
...
nohup torchrun --nproc_per_node="${GPU_COUNT}" biencoder_model_with_example.py \
  --data-dir data \
  ...
  --use-tensorboard \
  --tensorboard-logdir runs/stage1_biencoder_with_example \
  --best-model-dir best_model_with_example \
  --final-model-dir final_model_with_example \
  --num-epochs 50 \
  --batch-size 16 \
  --learning-rate 2e-5 \
  --max-length 256 \
